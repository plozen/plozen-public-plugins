...
description: 저장소 변경, 새 기능, 동작 변경, 버그 수정, 테스트 실패, 다단계 작업, 작업 범위 분류, 스킬 라우팅, planning, worktree, background subagent 디스패치, debugging/TDD, 리뷰/QA/security gate, verification, PR/merge/cleanup 통합이 필요할 때 사용한다. 사용자가 Aegis나 병렬 디스패치를 명시하지 않아도 경량 답변을 넘어서면 먼저 적용 범위를 분류하고, 실행 산출물이 생기는 작업은 단일 실행 단위여도 background dispatch를 기본값으로 한다.
---
...
- 경량 작업: 질문 답변, 범위 분류, 설계 대화, 상태 확인처럼 실행 산출물이 없는 작업은 현재 세션에서 바로 처리한다. Brainstorming, 작업 트리, 위임, PR을 강제하지 않는다.
- 표준 작업: 일반 기능 수정, 버그 수정, 테스트 추가, 문서 수정처럼 실행 산출물이 생기지만 위험이 낮은 작업은 단일 실행 단위여도 Background Dispatch Hook을 적용한다. 작업 트리, 커밋, 푸시, PR은 사용자 요청이나 저장소 정책이 있을 때 사용한다.
...
팀장은 실행자가 아니라 라우터, gate 판정자, 통합자다. 실행은 가능한 역할 에이전트나 독립 실행 컨텍스트가 맡는다. 최종 저장소 통합, 병합, 최종 푸시, 깨끗한 작업 트리 정리는 팀장이 책임진다.
...
- 위임: 실행 단위가 하나라도 있으면 [Background Dispatch Hook](#background-dispatch-hook)을 적용한다. 단일 실행 단위는 subagent 1개로 위임하고, 독립 실행 단위가 여러 개면 background subagent 병렬 디스패치를 수행한다.
...
역할 위임은 실행 산출물이 생기는 모든 표준/보호 작업에서 기본값이다. 단일 실행 단위여도 subagent 1개에 위임하고, 독립 실행 단위가 여러 개면 사용자가 매번 병렬 디스패치를 명시하지 않아도 background subagent 병렬 디스패치를 수행한다. 상위 지침, 도구 정책, 실행 환경이 위임을 금지하면 작업을 분해한 뒤 로컬 실행 계획으로 전환하고 생략 이유를 남긴다.
...
파일 수정, 코드/문서 작성, 조사, 테스트, 검증, 정리처럼 실행 산출물이 생기는 작업은 현재 실행 환경과 상위 지침이 허용하면 자동으로 하위 에이전트에 위임한다. 상위 지침, 도구 정책, 현재 런타임이 subagent 실행을 막을 때만 로컬 실행/검증으로 대체할지 판단하고, 생략 이유와 남은 위험을 보고한다.
...
실행 산출물이 생기는 표준/보호 작업은 실행 계획으로 들어가기 전에 작업을 분해한다. 실행 단위가 하나라도 있으면 background subagent 또는 동등한 실행 컨텍스트 위임을 기본값으로 한다. 단일 실행 단위는 subagent 1개에 맡기고, 독립 실행 단위가 여러 개면 병렬 디스패치한다.

- 팀장은 설계, 범위 분류, 작업 분해, 역할 선택, 위임, 통합, gate 판정을 맡는다.
- 실행 단위가 하나라도 있으면 디스패치 계획을 만들고 즉시 실행한다. 실행 단위가 여러 개면 병렬로 실행한다.
- 구현, 문서 작성, 조사, QA, 리뷰, 보안 점검, 브라우저 검증은 적절한 역할 에이전트에 디스패치한다.
- 독립적인 sidecar 작업은 위임이 허용되면 병렬로 디스패치하고, 팀장은 기다리는 동안 다른 설계/통합 작업을 계속한다.
- 즉시 다음 판단이 막히는 critical path 작업도 가능한 경우 foreground subagent에 맡기고 팀장은 결과를 기다린다.
...
- 완료, fixed, 통과, ready를 말하기 전에 증명할 명령이나 절차를 실행 단위로 만들고, 가능한 경우 검증 실행을 subagent에 위임한다.
