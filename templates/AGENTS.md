...
- 입력 프롬프트 끝의 `-bd`는 background dispatch/subagent 위임 의도를 명시한 플래그로 본다. 역할 선택, 실행 단위 분해, 병렬 여부, gate/검증 구성은 리드 에이전트가 판단하며, 실행 산출물이 없으면 디스패치할 실행 단위가 없다고 보고한다.
- 경량 작업은 `분류 -> 바로 답변/상태 확인 -> 필요한 근거 표시`로 처리한다.
- 표준 작업은 `분류 -> git/worktree 상태 확인 -> 수정/실행 -> 검증 -> 보고`로 처리한다.
- 표준 작업에서 `-bd`가 있거나 독립 실행 단위가 2개 이상이면 Background Dispatch Hook을 적용한다. 회귀 위험이 있으면 reviewer 또는 QA gate를 적용하고, 사용자 요청이나 저장소 정책이 있으면 commit/PR을 적용한다.
